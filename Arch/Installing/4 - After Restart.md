...
6) Time to install a bunch of packages ```pacman -Sy --needed --noconfirm xorg zsh git budgie-desktop gdm nemo chromium gnome-terminal guake vim gufw gnome gnome-backgrounds gnome-control-center gnome-screensaver cpu-power```
...
11) run AurSetup.sh to install pacaur
	...
	* Some general programs, add ```steam vlc libreoffice-fresh jre8-openjdk```
	* Some playback codecs for music / videos ```gst-libav gst-plugins-bad gst-plugins-base gst-plugins-good gst-plugins-ugly flac```
10) Continue to Part 5, more setup
