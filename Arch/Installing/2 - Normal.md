...
2) Use ```cfdisk /dev/sdX``` to Partition the deivce as you wish
	1) Partition strategies - Need ~500mb boot partition (if your using UEFI, 2-3 MB if your using bios boot), the rest is up to you.
		...
		2) Strategy 2 [device - size [mount point], [format to]
		...
		3) Strategy 3 - "Split everything up approach" - You should probably look at LVM if your going to do this, also additionally [this](https://security.stackexchange.com/a/38803), if you want to be a tiny bit more secure [device - size [mount point], [format to]
			* /dev/sdX1 - 500 MB /boot, see above
			* /dev/sdX2 - [size of ram], see swap above
...
3) After using lsblk to format to whatever your strategy is, you have to format the drives
...
4) Mount the partitions to the designated spots
...
5) Next, move on to ```3 - Pacstrap and Chroot```
